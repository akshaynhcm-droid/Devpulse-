...
export function validateEnv(): { valid: boolean; warnings: string[]; errors: string[] } {
  ...
  if (ENV.isProduction) {
    ...
    if (!ENV.googleClientSecret) errors.push("GOOGLE_CLIENT_SECRET is not set — OAuth login will fail");
    if (!ENV.razorpayKeyId) errors.push("RAZORPAY_KEY_ID is not set — payments will not work");
    if (!ENV.razorpayKeySecret) errors.push("RAZORPAY_KEY_SECRET is not set — payments will not work");
    if (!ENV.razorpayWebhookSecret) errors.push("RAZORPAY_WEBHOOK_SECRET is not set — webhook verification will fail");
  ...
  if (!ENV.slackWebhookUrl) warnings.push("SLACK_WEBHOOK_URL is not set — kill switch alerts will not be sent to Slack");
  ...
  if (!ENV.githubWebhookSecret) warnings.push("GITHUB_WEBHOOK_SECRET is not set — GitHub webhook integration disabled");
