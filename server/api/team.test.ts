...
describe('Team Router', () => {
  describe('invite', () => {
    ...
    it('should reject invitation for already invited email', async () => {
      ...
      getTeamMemberByEmail.mockResolvedValue({ id: 'existing', email: 'user@example.com' });
      ...
      expect(() => validateNewInvitation(existing)).toThrow('An invitation has already been sent to this email address');
    });
    ...
    it('should handle email send failure gracefully', async () => {
      ...
      await expect(sendTeamInviteEmail({
  ...
  describe('list', () => {
    it('should list team members with pagination', async () => {
      const mockMembers = [
        { id: 'm1', memberEmail: 'user1@example.com', role: 'admin', status: 'active', invitedAt: new Date() },
        { id: 'm2', memberEmail: 'user2@example.com', role: 'editor', status: 'active', invitedAt: new Date() },
        { id: 'm3', memberEmail: 'user3@example.com', role: 'viewer', status: 'pending', invitedAt: new Date() },
      ];
  ...
  describe('updateRole', () => {
    ...
    it('should prevent IDOR on role update', async () => {
      ...
      expect(() => validateMemberAccess(member, ctx.user.id)).toThrow('Team member not found or access denied');
    });
  ...
  describe('acceptInvitation', () => {
    ...
    it('should reject acceptance if email does not match', async () => {
      ...
      expect(() => validateInvitationEmail(member, ctx.user.email)).toThrow('This invitation is not for your email address');
    });
