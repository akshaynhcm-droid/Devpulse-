...
vi.mock(\"../payments\", () => ({
  ...
  PLAN_CONFIG: {
    ...
    enterprise: { name: \"Enterprise\", amount: 499900, features: [\"All features\"] },
  },
}));
...
describe(\"payments router\", () => {
  describe(\"createSubscription\", () => {
    it(\"throws when user is not authenticated\", async () => {
      const ctx = { user: null, req: { protocol: \"https\", headers: {}, ip: \"127.0.0.1\" }, res: { clearCookie: vi.fn() } } as unknown as TrpcContext;
    ...
    it(\"successfully creates enterprise subscription\", async () => {
      ...
      const result = await caller.payment.createSubscription({ plan: \"enterprise\" });
