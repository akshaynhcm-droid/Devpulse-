...
vi.mock(\"../db\", async () => {
  ...
  return {
    ...
    getUserById: vi.fn(async () => ({ id: 1, email: \"test@example.com\", name: \"Test User\" })),
  };
});
...
vi.mock(\"../services/scanService\", () => ({
  runCollectionScan: vi.fn(async () => ({
    ...
    totalFindings: 3,
    findings: [
      { id: \"f1\", title: \"Test Finding\", severity: \"Medium\", description: \"Test\", category: \"Test\", remediation: \"Fix it\", cweId: \"CWE-000\" }
    ]
  }))
}));
...
describe(\"scanning router\", () => {
  ...
  describe(\"startScan\", () => {
    ...
    it(\"successfully starts a scan on owned collection\", async () => {
      ...
      const result = await caller.scanning.startScan({ collectionId: \"col_123\", scanType: \"full\" });
    ...
    it(\"starts a shadow_api scan type\", async () => {
      ...
      const result = await caller.scanning.startScan({ collectionId: \"col_123\", scanType: \"shadow_api\" });
  ...
  describe(\"listScans\", () => {
    it(\"throws when user is not authenticated\", async () => {
      ...
      await expect(
        caller.scanning.listScans({ collectionId: \"col_123\", page: 1, pageSize: 20 })
      ).rejects.toThrow(\"UNAUTHORIZED\");
    });
    ...
    it(\"returns empty list for collection with no scans\", async () => {
      ...
      vi.mocked(getCollectionById).mockResolvedValueOnce({ id: \"col_123\", userId: 1 });
      vi.mocked(getScansByCollectionId).mockResolvedValueOnce([]);
      
      const result = await caller.scanning.listScans({ collectionId: \"col_123\" });
    ...
    it(\"filters by scanType when specified\", async () => {
      ...
      vi.mocked(getCollectionById).mockResolvedValueOnce({ id: \"col_123\", userId: 1 });
      vi.mocked(getScansByCollectionId).mockResolvedValueOnce([
        { id: \"scan1\", scanType: \"full\", status: \"completed\", riskScore: \"30\", riskLevel: \"MEDIUM\", totalFindings: 2 },
        { id: \"scan2\", scanType: \"quick\", status: \"completed\", riskScore: \"20\", riskLevel: \"LOW\", totalFindings: 1 },
        { id: \"scan3\", scanType: \"full\", status: \"completed\", riskScore: \"45\", riskLevel: \"MEDIUM\", totalFindings: 3 }
      ]);
      
      const result = await caller.scanning.listScans({ collectionId: \"col_123\", scanType: \"full\" });
  ...
  describe(\"updateFindingStatus\", () => {
    it(\"throws when user is not authenticated\", async () => {
      ...
      await expect(
        caller.scanning.updateFindingStatus({ findingId: \"f_123\", status: \"resolved\" })
      ).rejects.toThrow(\"UNAUTHORIZED\");
    });
...
describe(\"scanning edge cases\", () => {
  it(\"handles scan with empty collection data\", async () => {
    ...
    const result = await caller.scanning.startScan({ collectionId: \"col_empty\", scanType: \"full\" });
  ...
  it(\"handles Postman collection format\", async () => {
    ...
    const result = await caller.scanning.startScan({ collectionId: \"col_postman\", scanType: \"full\" });
  ...
  it(\"handles OpenAPI format\", async () => {
    ...
    const result = await caller.scanning.startScan({ collectionId: \"col_openapi\", scanType: \"full\" });
  ...
  it(\"caps risk score at 100\", async () => {
    ...
    vi.mocked(getCollectionById).mockResolvedValueOnce({
      ...
      name: \"Many Issues\",
      data: {
        item: Array(20).fill(null).map((_, i) => ({
          request: {
            method: \"POST\",
    ...
    const result = await caller.scanning.startScan({ collectionId: \"col_many\", scanType: \"full\" });
