    ...
    <div className="min-h-screen bg-gradient-to-br from-background to-muted">
      ...
      <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-20 space-y-8">
        <div className="space-y-6 text-center">
          ...
          <p className="text-xl text-muted-foreground max-w-2xl mx-auto">
            Unified platform for managing API vulnerabilities, compliance, and autonomous agent spending
          </p>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mt-16">
          {[
            { icon: Shield, title: "Security Scanning", description: "OWASP vulnerability detection and analysis" },
            { icon: Zap, title: "Kill Switch", description: "Budget limits and emergency LLM controls" },
            { icon: BarChart3, title: "Token Analytics", description: "Track AI model usage and costs" },
            { icon: Users, title: "Team Collaboration", description: "Role-based access and team management" },
          ].map((feature) => {
            const Icon = feature.icon;
            return (
              <div key={feature.title} className="rounded-lg border border-border/50 bg-card p-6 space-y-3 hover:shadow-md transition-all">
      ...
      <section className="bg-card border-t border-border/50 py-20">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-12">
          <div className="text-center space-y-4">
            <h2 className="text-4xl font-bold text-foreground">Complete API Security Platform</h2>
            <p className="text-lg text-muted-foreground">Everything you need to secure and manage your APIs</p>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-12">
            {[
              {
                title: "API Collection Management",
                items: ["Import Postman and OpenAPI collections", "Organize and manage multiple collections", "Track API endpoints and versions"],
              },
              {
                title: "Security Intelligence",
                items: ["OWASP Top 10 vulnerability scanning", "Shadow API detection", "Risk scoring and severity levels"],
              },
              {
                title: "Compliance & Reporting",
                items: ["PCI DSS compliance reports", "Automated compliance tracking", "Detailed requirement breakdowns"],
              },
              {
                title: "LLM Cost Management",
                items: ["Token analytics with MiniMax M2.7", "Cost breakdown by model", "Budget limits and kill switch"],
              },
            ].map((section) => (
      ...
      <section className="py-16 bg-gradient-to-r from-blue-600/5 to-violet-600/5 border-y border-border/50">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 text-center space-y-4">
          ...
          <h2 className="text-3xl font-bold text-foreground">Powered by MiniMax M2.7</h2>
          <p className="text-lg text-muted-foreground max-w-2xl mx-auto">
            DevPulse uses MiniMax M2.7 — a cutting-edge reasoning model — as its core intelligence engine.
            Every scan analysis, compliance mapping, and threat explanation is generated with deep reasoning
            and automatically tracked in your token analytics dashboard.
          </p>
          <div className="flex flex-wrap items-center justify-center gap-6 pt-2 text-sm text-muted-foreground">
            <span className="flex items-center gap-1.5"><span className="w-2 h-2 rounded-full bg-green-500" />OpenAI-compatible API</span>
            <span className="flex items-center gap-1.5"><span className="w-2 h-2 rounded-full bg-blue-500" />Chain-of-thought reasoning</span>
            <span className="flex items-center gap-1.5"><span className="w-2 h-2 rounded-full bg-violet-500" />Auto cost tracking</span>
          </div>
        </div>
      </section>
      ...
      <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-20 text-center space-y-6">
        <h2 className="text-4xl font-bold text-foreground">Ready to secure your APIs?</h2>
        <p className="text-lg text-muted-foreground">Start with a free account and get full access to all features</p>
      ...
      <footer className="border-t border-border/50 bg-card/50 py-8">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center text-muted-foreground text-sm">
          <p>© 2026 DevPulse. All rights reserved. · Powered by MiniMax M2.7</p>
        </div>
      </footer>
    </div>
  );
