...
export default function Scanning() {
  ...
  const { data: scans, isLoading: scansLoading, refetch: refetchScans } =
    trpc.scanning.listScans.useQuery(
      { collectionId, scanType: "full" },
      { enabled: !!collectionId }
    );
  const { data: currentScan, refetch: refetchCurrentScan } = trpc.scanning.getScan.useQuery(
  ...
  const handleUpdateStatus = async (findingId: string, status: "open" | "in-progress" | "resolved") => {
    try {
  ...
  const filteredFindings = currentScan?.findings?.filter((f: any) =>
        ...
        <div className="space-y-4">
          ...
          {scansLoading ? (
            ...
            <Card className="p-8 text-center">
              <p className="text-muted-foreground">No scans yet. Click "Start Scan" to begin.</p>
            </Card>
          )}
        </div>
      )}
      ...
      {selectedScanId && currentScan && (
        <div className="space-y-4">
            ...
            <div className="flex gap-2">
              {["all", "Critical", "High", "Medium", "Low"].map((severity) => (
                <Button
                  ...
                  size="sm"
                  onClick={() => setSelectedSeverity(severity as SeverityLevel | "all")}
                >
          ...
          {filteredFindings.length > 0 ? (
              ...
              {filteredFindings.map((finding: any) => (
                <Card key={finding.id} className="p-6 space-y-4">
                  <div className="flex items-start justify-between">
                    <div className="flex-1">
                      <h3 className="font-semibold text-foreground text-lg">{finding.title}</h3>
                      <p className="text-sm text-muted-foreground mt-1">{finding.description}</p>
                    </div>
                    <span className={`inline-flex items-center rounded-full px-3 py-1 text-xs font-semibold ${severityColor(finding.severity)}`}>
                      {finding.severity}
                    </span>
                  </div>
                  ...
                  {finding.category && (
                    <div>
                      ...
                      <p className="text-sm text-foreground">{finding.category}</p>
                    </div>
                  )}
                  ...
                  {finding.cweId && (
                    <div>
                      <p className="text-xs font-medium text-muted-foreground">CWE ID</p>
                    ...
                    <div className="flex gap-2 ml-auto">
                      {finding.status !== "in-progress" && (
                        <Button
                          size="sm"
                          variant="outline"
                          onClick={() => handleUpdateStatus(finding.id, "in-progress")}
                        >
                          In Progress
                        </Button>
                      )}
            ...
            <Card className="p-8 text-center">
              <p className="text-muted-foreground">No findings with selected severity level.</p>
            </Card>
          )}
        </div>
      )}
