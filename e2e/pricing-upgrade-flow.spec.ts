...
test.describe("Critical Path 4: Pricing & Upgrade Flow", () => {
  test("Login → Go to pricing → Upgrade plan → Verify feature gating removed", async ({ page }) => {
    ...
    await expect(page.getByText(/upgrade to unlock|pro feature|limited/i)).toBeVisible();
    ...
    await expect(page.getByRole("heading", { name: /checkout|payment|upgrade/i })).toBeVisible();
    ...
    // Should show success
    await expect(page.getByText(/success|confirmed|welcome to pro/i)).toBeVisible({ timeout: 10000 });
