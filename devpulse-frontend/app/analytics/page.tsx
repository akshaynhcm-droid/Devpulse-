      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
          <div className="bg-gray-800 p-6 rounded-lg border border-gray-700">
            <h3 className="text-gray-400 text-sm uppercase tracking-wide">Total Cost</h3>
            <p className="text-4xl font-bold mt-2 text-green-400">
              ${(data?.total_cost || 0).toFixed(4)}
            </p>
          </div>
          <div className="bg-gray-800 p-6 rounded-lg border border-gray-700">
            <h3 className="text-gray-400 text-sm uppercase tracking-wide">API Calls</h3>
            <p className="text-4xl font-bold mt-2 text-blue-400">
              {data?.recent_calls?.length || 0}
            </p>
          </div>
          <div className="bg-gray-800 p-6 rounded-lg border border-gray-700">
            <h3 className="text-gray-400 text-sm uppercase tracking-wide">Security Events</h3>
            ...
            <div className="space-y-2 max-h-96 overflow-y-auto">
              {data?.recent_calls?.length === 0 ? (
                ...
                data?.recent_calls?.map((call, i) => (
                  <div key={i} className="flex justify-between items-center bg-gray-700/50 p-3 rounded">
                    <div>
                      <span className="text-blue-300 font-mono text-sm">{call.model}</span>
                      <span className="text-gray-500 text-xs ml-2">{call.agent_id}</span>
                    </div>
                    <span className="text-green-400">${(call.cost_usd || 0).toFixed(6)}</span>
                  </div>
                ))
              )}
            </div>
